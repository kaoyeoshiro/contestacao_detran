...
def test_allowed_file_accepts_pdf():
    ...
    assert module.PDFProcessor.allowed_file("doc.pdf") is True
    assert module.PDFProcessor.allowed_file("doc.PDF") is True


def test_allowed_file_rejects_non_pdf():
    module = import_backend_module()
    assert module.PDFProcessor.allowed_file("image.png") is False
    assert module.PDFProcessor.allowed_file("file") is False

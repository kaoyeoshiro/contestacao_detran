...
def index():
    try:
        if request.method == "GET": return _handle_get_request()
        elif request.method == "POST": return _handle_post_request()
        logger.warning(f"Método não permitido: {request.method} para {request.url}")
        return html_generator_instance.generate_page(erro_msg="Método não permitido."), 405
    except Exception as e:
        logger.error(f"Erro rota principal {request.url}: {e}", exc_info=True)
        return html_generator_instance.generate_page(erro_msg="Erro interno. Logs consultados.")

def _handle_get_request():
    logger.debug(f"GET {request.path}, Session ID: {session.sid if session.sid else 'N/A'}")
    minuta_disp_data = None
    if 'minuta_gerada' in session and session['minuta_gerada']: # Checa se existe e não é None/vazio
        minuta_disp_data = minuta_parser_instance.parse_minuta_to_single_block(session['minuta_gerada'])
    
    return html_generator_instance.generate_page(
        minuta_data=minuta_disp_data, 
        filenames_processados=session.get('filenames_processados'),
        # temp_text_file_id não é mais necessário aqui
        erro_msg=session.pop('erro_msg', None), 
        sucesso_msg=session.pop('sucesso_msg', None),
        warnings=session.pop('warnings', None)
    )

def _handle_post_request():
    action = request.form.get("action")
    logger.debug(f"POST {request.path} Action: {action}, Session ID: {session.sid if session.sid else 'N/A'}")
    if action == "upload_pdfs": return _handle_upload_pdfs()
    elif action == "ajustar_minuta": return _handle_ajustar_minuta()
    session['erro_msg'] = "Ação inválida."
    logger.warning(f"Ação POST desconhecida: {action}")
    return redirect(url_for("index"))

def _handle_upload_pdfs():
    logger.info("Processando upload de PDFs.")
    session.clear() # Limpa toda a sessão anterior para um novo upload
    
    if 'pdfs' not in request.files:
        session['erro_msg'] = "Nenhum PDF enviado."; return redirect(url_for("index"))
    files = request.files.getlist('pdfs')
    if not files or all(f.filename == '' for f in files):
        session['erro_msg'] = "Nenhum PDF selecionado."; return redirect(url_for("index"))
    if len(files) > MAX_FILES:
        session['erro_msg'] = f"Máximo {MAX_FILES} arquivos."; return redirect(url_for("index"))
    ...
    for f in files:
        ...
        if f and f.filename and pdf_processor_instance.allowed_file(f.filename):
            ...
            if f_size > MAX_FILE_SIZE: val_errors.append(f"'{s_fn}' excede limite.")
            else: valid_files.append(f)
        elif f and f.filename: val_errors.append(f"'{s_fn}' não é PDF.")
    if val_errors: session['erro_msg'] = " ".join(val_errors); return redirect(url_for("index"))
    if not valid_files: session['erro_msg'] = "Nenhum PDF válido."; return redirect(url_for("index"))
        
    texto_pdfs, filenames, extract_errors = pdf_processor_instance.extract_text_from_pdfs(valid_files)
    
    current_warnings = [] # Inicializa warnings para esta requisição
    if extract_errors: current_warnings.extend(extract_errors)

    if not texto_pdfs:
        err = "Não foi possível extrair texto dos PDFs." + (f" Detalhes: {'; '.join(extract_errors)}" if extract_errors else "")
        session['erro_msg'] = err; 
        if current_warnings: session['warnings'] = current_warnings
        return redirect(url_for("index"))
    
    # Com Flask-Session, podemos salvar diretamente na sessão.
    session['texto_pdfs_original'] = texto_pdfs 
    session['filenames_processados'] = filenames
    if current_warnings: session['warnings'] = current_warnings

    if not model:
        session['erro_msg'] = "Erro crítico: O serviço de IA não está configurado."; return redirect(url_for("index"))

    logger.info("Chamando gerador de minutas.")
    ...
    if isinstance(minuta_gerada, str) and minuta_gerada.startswith("Erro:"):
        session['erro_msg'] = minuta_gerada
    else:
        session['sucesso_msg'] = "Minuta gerada com sucesso!"
        session['minuta_gerada'] = minuta_gerada # Salva na sessão (lado do servidor)
        
    return redirect(url_for("index"))

def _handle_ajustar_minuta():
    logger.info("Iniciando ajuste de minuta.")
    instrucoes = request.form.get("instrucoes_ajuste", "").strip()
    
    # Agora lemos 'texto_pdfs_original' diretamente da sessão (lado do servidor)
    texto_original_final = session.get("texto_pdfs_original")

    if not texto_original_final:
        session['erro_msg'] = "Sessão expirada ou texto original não encontrado. Faça um novo upload."
        logger.warning("Texto original para ajuste não encontrado na sessão.")
        return redirect(url_for("index"))

    if not instrucoes:
        session['erro_msg'] = "Forneça instruções para o ajuste."; return redirect(url_for("index"))
    if not model:
        session['erro_msg'] = "Erro: Serviço de IA não configurado."; return redirect(url_for("index"))
    
    logger.info(f"Ajustando minuta com instruções: '{instrucoes[:100]}...'")
    ...
    if isinstance(nova_minuta, str) and nova_minuta.startswith("Erro:"):
        session['erro_msg'] = f"Falha no ajuste: {nova_minuta}"
    else:
        session['sucesso_msg'] = "Minuta ajustada com sucesso!"
        session['minuta_gerada'] = nova_minuta # Atualiza na sessão (lado do servidor)
        
    return redirect(url_for("index"))

# --- Tratamento de Erros HTTP ---
@app.errorhandler(404)
def not_found_error(error): logger.warning(f"404: {request.url}"); return html_generator_instance.generate_page(erro_msg="Página não encontrada (404)."), 404
@app.errorhandler(500)
def internal_error(error): logger.error(f"500: {error}", exc_info=True); return html_generator_instance.generate_page(erro_msg="Erro interno (500)."), 500
@app.errorhandler(413) 
def too_large_error(error): max_mb = app.config.get('MAX_CONTENT_LENGTH',MAX_FILES*MAX_FILE_SIZE)/(1024*1024); return html_generator_instance.generate_page(erro_msg=f"Arquivos grandes. Limite: {max_mb:.1f}MB."), 413
